...
KEYSTORE_PATH = Path(RELTOOLS_ROOT / 'signapp-keystore.jks')


class AlignApk:
    def __init__(self, input_apk_path, output_apk_path):
        run_process(f'zipalign -p -f -v 4 {input_apk_path} {output_apk_path}')
...
def parse_arguments():
    ...
    parser.add_argument('-a', '--apk-path', required=True, help='unsigned apk path')
...
def sign_app(output_apk_path, apk_path, keystore_path=None, install_after_build=False):
    AlignApk(apk_path, output_apk_path)
...
def main():
    ...
    sign_app(args.output_apk_path, args.apk_path, args.keystore_path, args.install)

    print('signapp completed successfully!')
